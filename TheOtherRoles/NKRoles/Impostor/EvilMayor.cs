...
public static class EvilMayor
{
    ...
    {
        var writer = AmongUsClient.Instance.StartRpcImmediately(PlayerControl.LocalPlayer.NetId, (byte)CustomRPC.unlockEvilMayorAcCommon, SendOption.Reliable, -1);
        writer.Write(votedFor);
        AmongUsClient.Instance.FinishRpcImmediately(writer);
        RPCProcedure.unlockEvilMayorAcCommon(votedFor);
    }
    ...
    public static void clearAndReload()
    {
        ...
        remoteMeetingsLeft = Mathf.RoundToInt(CustomOptionHolder.evilmayorMaxRemoteMeetings.getFloat());
        meetingButton = CustomOptionHolder.evilmayorMeetingButton.getBool();
        numVotes = (int)CustomOptionHolder.evilmayorNumVotes.getFloat();
